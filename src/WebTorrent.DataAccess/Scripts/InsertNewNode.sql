...
BEGIN
	...
	ELSE
    BEGIN
		SELECT @nodeId = rgt FROM dbo.Content WHERE Id = @pparent_id;
				UPDATE dbo.Content SET rgt = rgt + 2 WHERE rgt >= @nodeId;
				UPDATE dbo.Content SET lft = lft + 2 WHERE lft > @nodeId;
			INSERT INTO dbo.Content (lft, rgt, parentId) VALUES (@nodeId, (@nodeId + 1), @pparent_id);
			SELECT @nodeId = SCOPE_IDENTITY();
	END
END